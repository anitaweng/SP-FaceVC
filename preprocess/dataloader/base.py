...
class BaseDataset(Dataset):
    def __init__(self, dset, indexes_path, feat, feat_path, seglen, pitch_path, pitchlen, ap_path, face_path, sp_path, njobs, metadata):
        ...
        if metadata is None:
            data = self.gen_metadata(dset, indexes_path, feat, feat_path, njobs, pitch_path, ap_path, face_path, sp_path)
        else:
            data = metadata
        
        self.seglen = seglen
        self.pitchlen = pitchlen
        self.data = BaseDataset.drop_invalid_data(data, verbose=True)
        self.speaker2data = BaseDataset.gen_speaker2data(self.data)

    def gen_metadata(self, dset, indexes_path, feat, feat_path, njobs, pitch_path, ap_path, face_path, sp_path):
        ...
        task = partial(self.sub_process, feat=feat, feat_path=feat_path, pitch_path=pitch_path, ap_path=ap_path, face_path=face_path, sp_path=sp_path)
